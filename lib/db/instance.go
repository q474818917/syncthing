...
func (db *instance) updateRemoteFiles(folder, device []byte, fs []protocol.FileInfo, meta *metadataTracker) {
	...
	for _, f := range fs {
		name := []byte(f.Name)
		dk = db.keyer.GenerateDeviceFileKey(dk, folder, device, name)	//dk -> device key（dk-fileInfo）
		...
		gk = db.keyer.GenerateGlobalVersionKey(gk, folder, name)		//gk -> global key（gk-versionList）
